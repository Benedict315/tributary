...
import { toStroops, fromStroops } from "./tributary";
...
describe("Token conversion functions", () => {
  ...
  describe("fromStroops", () => {
    it("converts 7-decimal token correctly", () => {
      expect(fromStroops(10_000_000n, 7)).toBe("10,000,000");
      expect(fromStroops(5_000_000n, 7)).toBe("5,000,000");
      expect(fromStroops(12_345_678n, 7)).toBe("12,345,678");
      expect(fromStroops(1n, 7)).toBe("0.0000001");
    });

    it("converts 6-decimal token correctly", () => {
      expect(fromStroops(1_000_000n, 6)).toBe("1,000,000");
      expect(fromStroops(500_000n, 6)).toBe("500,000");
      expect(fromStroops(1_234_567n, 6)).toBe("1,234,567");
      expect(fromStroops(1n, 6)).toBe("0.000001");
    });

    it("converts 18-decimal token correctly", () => {
      expect(fromStroops(1_000_000_000_000_000_000n, 18)).toBe("1,000,000,000,000,000,000");
      expect(fromStroops(500_000_000_000_000_000n, 18)).toBe("500,000,000,000,000,000");
    ...
    it("defaults to 7 decimals", () => {
      expect(fromStroops(10_000_000n)).toBe("10,000,000");
      expect(fromStroops(5_000_000n)).toBe("5,000,000");
    });
  });
  ...
  describe("round-trip conversion", () => {
    ...
    it("round-trips 18-decimal token correctly", () => {
      ...
      expect(back).toBe("1,234,567,890,123,456,789");
    });
    ...
    it("round-trips XLM (7 decimals) correctly", () => {
      ...
      const back = fromStroops(stroops, 7);
      expect(back).toBe("1,005,000,000");
    });
  });
});
